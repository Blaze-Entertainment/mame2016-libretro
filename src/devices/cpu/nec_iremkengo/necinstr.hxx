...
#define OP(num,func_name) void nec_common_iremkengo_device::func_name()
...
OP( 0x0f, i_pre_nec  ) { UINT32 ModRM, tmp, tmp2;
	switch (FETCH()) {
		...
		case 0xe0 : ModRM = FETCH(); ModRM=0; logerror("%06x: V33IREMKENGO unimplemented BRKXA (break to expansion address)\n",PC()); break;
		case 0xf0 : ModRM = FETCH(); ModRM=0; logerror("%06x: V33IREMKENGO unimplemented RETXA (return from expansion address)\n",PC()); break;
		case 0xff : ModRM = FETCH(); ModRM=0; logerror("%06x: unimplemented BRKEM (break to 8080 emulation mode)\n",PC()); break;
		default:    logerror("%06x: Unknown V20IREMKENGO instruction\n",PC()); break;
	}
}
...
OP( 0x62, i_chkind  ) {
	...
	tmp= RegWord(ModRM);
	if (tmp<low || tmp>high) {
		nec_interrupt(NEC_IREMKENGO_CHKIND_VECTOR, BRK);
	}
	m_icount-=20;
...
OP( 0xce, i_into      ) { if (OF) { nec_interrupt(NEC_IREMKENGO_BRKV_VECTOR, BRK); CLKS(52,52,26); } else CLK(3); }
...
OP( 0xf6, i_f6pre ) { UINT32 tmp; UINT32 uresult,uresult2; INT32 result,result2;
	...
	switch (ModRM & 0x38) {
		...
		case 0x30: if (tmp) { DIVUB; } else nec_interrupt(NEC_IREMKENGO_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
		case 0x38: if (tmp) { DIVB;  } else nec_interrupt(NEC_IREMKENGO_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
	}
}
...
OP( 0xf7, i_f7pre   ) { UINT32 tmp,tmp2; UINT32 uresult,uresult2; INT32 result,result2;
	...
	switch (ModRM & 0x38) {
		...
		case 0x30: if (tmp) { DIVUW; } else nec_interrupt(NEC_IREMKENGO_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
		case 0x38: if (tmp) { DIVW;  } else nec_interrupt(NEC_IREMKENGO_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
	}
}
...
void nec_common_iremkengo_device::i_invalid()
{
	m_icount-=10;
