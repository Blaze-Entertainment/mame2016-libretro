...
void sh2alt_device::sh2alt_timer_resync()
{
...
void sh2alt_device::sh2alt_timer_activate()
{
	...
	if(max_delta != 0xfffff) {
		...
		if(divider) {
			...
			logerror("SH2ALT.%s: Timer event in %d cycles of external clock", tag(), max_delta);
		}
	}
}

TIMER_CALLBACK_MEMBER( sh2alt_device::sh2alt_timer_callback )
{
	UINT16 frc;
...
/*
  ...
  time as the SH2ALT is transfering it out via DMA
...
void sh2alt_device::sh2alt_notify_dma_data_available()
{
	//printf("call notify\n");
...
void sh2alt_device::sh2alt_do_dma(int dma)
{
	UINT32 dmadata;
		...
		case 3:
			{
				...
				if (!m_dma_fifo_data_available_cb.isnull())
				{
					...
					if (!available)
					{
						//printf("dma stalled\n");
						m_dma_timer_active[dma]=2;// mark as stalled
						fatalerror("SH2ALT dma_callback_fifo_data_available == 0 in unsupported mode\n");
					}
				}
...
TIMER_CALLBACK_MEMBER( sh2alt_device::sh2alt_dma_current_active_callback )
{
	int dma = param & 1;
...
void sh2alt_device::sh2alt_dmac_check(int dma)
{
	...
	else
	{
		...
		{
			logerror("SH2ALT: DMA %d cancelled in-flight\n", dma);
...
WRITE32_MEMBER( sh2alt_device::sh7604_w )
{
	UINT32 old;
...
//      printf("SH2ALT.%s: TIER write %04x @ %04x\n", m_device->tag(), data >> 16, mem_mask>>16);
...
//      printf("SH2ALT.%s: TCR write %08x @ %08x\n", m_device->tag(), data, mem_mask);
		sh2alt_timer_resync();
		if(m_m[5] & 0x10)
	...
	default:
		logerror("sh2alt_internal_w:  Unmapped write %08x, %08x @ %08x\n", 0xfffffe00+offset*4, data, mem_mask);
		break;
	}
}

READ32_MEMBER( sh2alt_device::sh7604_r )
{
//  logerror("sh2alt_internal_r:  Read %08x (%x) @ %08x\n", 0xfffffe00+offset*4, offset, mem_mask);
	switch( offset )
	{
	case 0x00:
		break;
...
void sh2alt_device::sh2alt_set_frt_input(int state)
{
	if(state == PULSE_LINE)
	{
		sh2alt_set_frt_input(ASSERT_LINE);
		sh2alt_set_frt_input(CLEAR_LINE);
		return;
	}
	...
	m_icr = m_frc;
	m_m[4] |= ICF;
	//logerror("SH2ALT.%s: ICF activated (%x)\n", tag(), m_sh2alt_state.pc & AM);
	sh2alt_recalc_irq();
}

void sh2alt_device::sh2alt_recalc_irq()
{
	int irq = 0, vector = -1;
	int  level;
	...
	m_sh2alt_state.internal_irq_level = irq;
	m_internal_irq_vector = vector;
	m_test_irq = 1;
}

void sh2alt_device::sh2alt_exception(const char *message, int irqline)
{
	int vector;

	if (irqline != 16)
	{
		if (irqline <= ((m_sh2alt_state.sr >> 4) & 15)) /* If the cpu forbids this interrupt */
			return;

		// if this is an sh2alt internal irq, use its vector
		if (m_sh2alt_state.internal_irq_level == irqline)
		{
	...
	m_sh2alt_state.r[15] -= 4;
	WL( m_sh2alt_state.r[15], m_sh2alt_state.pc );     /* push PC onto stack */

	/* set I flags in SR */
	if (irqline > SH2ALT_INT_15)
		m_sh2alt_state.sr = m_sh2alt_state.sr | I;
	else
		m_sh2alt_state.sr = (m_sh2alt_state.sr & ~I) | (irqline << 4);

	/* fetch PC */
	m_sh2alt_state.pc = RL( m_sh2alt_state.vbr + vector * 4 );
	

	if(m_sh2alt_state.sleep_mode == 1) { m_sh2alt_state.sleep_mode = 2; }
}
...
void sh2alta_device::sh7032_dma_exec(int ch)
{
...
READ32_MEMBER(sh2alta_device::dma_sar0_r)
{
	return m_dma[0].sar;
}

WRITE32_MEMBER(sh2alta_device::dma_sar0_w)
{
	COMBINE_DATA(&m_dma[0].sar);
}

READ32_MEMBER(sh2alta_device::dma_dar0_r)
{
	return m_dma[0].dar;
}

WRITE32_MEMBER(sh2alta_device::dma_dar0_w)
{
	COMBINE_DATA(&m_dma[0].dar);
}

READ16_MEMBER(sh2alta_device::dma_tcr0_r)
{
	return m_dma[0].tcr;
}

WRITE16_MEMBER(sh2alta_device::dma_tcr0_w)
{
	//printf("%04x\n",data);
	COMBINE_DATA(&m_dma[0].tcr);
}

READ16_MEMBER(sh2alta_device::dma_chcr0_r)
{
	return m_dma[0].chcr;
}

WRITE16_MEMBER(sh2alta_device::dma_chcr0_w)
{
	//printf("%04x CHCR0\n",data);
...
READ16_MEMBER(sh2alta_device::dmaor_r)
{
	return m_dmaor;
}

WRITE16_MEMBER(sh2alta_device::dmaor_w)
{
...
/*!
  ...
  */
READ16_MEMBER(sh1alt_device::sh7032_r)
{
...
/*!
  @brief Dummy debug interface
 */
WRITE16_MEMBER(sh1alt_device::sh7032_w)
{
	COMBINE_DATA(&m_sh7032_regs[offset]);
}

READ16_MEMBER(sh2alta_device::sh7021_r)
{
...
/*!
  @brief Dummy debug interface
 */
WRITE16_MEMBER(sh2alta_device::sh7021_w)
{
