...
#define SH2ALT_INT_NONE    -1
#define SH2ALT_INT_VBLIN   0
#define SH2ALT_INT_VBLOUT  1
#define SH2ALT_INT_HBLIN   2
#define SH2ALT_INT_TIMER0  3
#define SH2ALT_INT_TIMER1  4
#define SH2ALT_INT_DSP     5
#define SH2ALT_INT_SOUND   6
#define SH2ALT_INT_SMPC    7
#define SH2ALT_INT_PAD     8
#define SH2ALT_INT_DMA2    9
#define SH2ALT_INT_DMA1    10
#define SH2ALT_INT_DMA0    11
#define SH2ALT_INT_DMAILL  12
#define SH2ALT_INT_SPRITE  13
#define SH2ALT_INT_14      14
#define SH2ALT_INT_15      15
#define SH2ALT_INT_ABUS    16

enum
{
	SH2ALT_PC=1, SH2ALT_SR, SH2ALT_PR, SH2ALT_GBR, SH2ALT_VBR, SH2ALT_MACH, SH2ALT_MACL,
	SH2ALT_R0, SH2ALT_R1, SH2ALT_R2, SH2ALT_R3, SH2ALT_R4, SH2ALT_R5, SH2ALT_R6, SH2ALT_R7,
	SH2ALT_R8, SH2ALT_R9, SH2ALT_R10, SH2ALT_R11, SH2ALT_R12, SH2ALT_R13, SH2ALT_R14, SH2ALT_R15, SH2ALT_EA
};


typedef device_delegate<int (UINT32 src, UINT32 dst, UINT32 data, int size)> sh2alt_dma_kludge_delegate;
#define SH2ALT_DMA_KLUDGE_CB(name)  int name(UINT32 src, UINT32 dst, UINT32 data, int size)

typedef device_delegate<int (UINT32 src, UINT32 dst, UINT32 data, int size)> sh2alt_dma_fifo_data_available_delegate;
#define SH2ALT_DMA_FIFO_DATA_AVAILABLE_CB(name)  int name(UINT32 src, UINT32 dst, UINT32 data, int size)

typedef device_delegate<void (UINT32 data)> sh2alt_ftcsr_read_delegate;
#define SH2ALT_FTCSR_READ_CB(name)  void name(UINT32 data)


#define MCFG_SH2ALT_IS_SLAVE(_slave) \
	sh2alt_device::set_is_slave(*device, _slave);

#define MCFG_SH2ALT_DMA_KLUDGE_CB(_class, _method) \
	sh2alt_device::set_dma_kludge_callback(*device, sh2alt_dma_kludge_delegate(&_class::_method, #_class "::" #_method, downcast<_class *>(owner)));

#define MCFG_SH2ALT_FIFO_DATA_AVAIL_CB(_class, _method) \
	sh2alt_device::set_dma_fifo_data_available_callback(*device, sh2alt_dma_fifo_data_available_delegate(&_class::_method, #_class "::" #_method, downcast<_class *>(owner)));

#define MCFG_SH2ALT_FTCSR_READ_CB(_class, _method) \
	sh2alt_device::set_ftcsr_read_callback(*device, sh2alt_ftcsr_read_delegate(&_class::_method, #_class "::" #_method, downcast<_class *>(owner)));
...
#define SH2ALTDRC_STRICT_VERIFY        0x0001          /* verify all instructions */
#define SH2ALTDRC_FLUSH_PC         0x0002          /* flush the PC value before each memory access */
#define SH2ALTDRC_STRICT_PCREL     0x0004          /* do actual loads on MOVLI/MOVWI instead of collapsing to immediates */

#define SH2ALTDRC_COMPATIBLE_OPTIONS   (SH2ALTDRC_STRICT_VERIFY | SH2ALTDRC_FLUSH_PC | SH2ALTDRC_STRICT_PCREL)
#define SH2ALTDRC_FASTEST_OPTIONS  (0)

#define SH2ALT_MAX_FASTRAM       4

class sh2alt_frontend;

class sh2alt_device : public cpu_device
{
	friend class sh2alt_frontend;

public:
	// construction/destruction
	sh2alt_device(const machine_config &mconfig, const char *_tag, device_t *_owner, UINT32 _clock);
	sh2alt_device(const machine_config &mconfig, device_type type, const char *name, const char *tag, device_t *owner, UINT32 clock, const char *shortname, const char *source, int cpu_type,address_map_constructor internal_map, int addrlines);

	static void set_is_slave(device_t &device, int slave) { downcast<sh2alt_device &>(device).m_is_slave = slave; }
	static void set_dma_kludge_callback(device_t &device, sh2alt_dma_kludge_delegate callback) { downcast<sh2alt_device &>(device).m_dma_kludge_cb = callback; }
	static void set_dma_fifo_data_available_callback(device_t &device, sh2alt_dma_fifo_data_available_delegate callback) { downcast<sh2alt_device &>(device).m_dma_fifo_data_available_cb = callback; }
	static void set_ftcsr_read_callback(device_t &device, sh2alt_ftcsr_read_delegate callback) { downcast<sh2alt_device &>(device).m_ftcsr_read_cb = callback; }

	DECLARE_WRITE32_MEMBER( sh7604_w );
	DECLARE_READ32_MEMBER( sh7604_r );
	DECLARE_READ32_MEMBER(sh2alt_internal_a5);


	void sh2alt_notify_dma_data_available();
...
private:
	...
	sh2alt_dma_kludge_delegate              m_dma_kludge_cb;
	sh2alt_dma_fifo_data_available_delegate m_dma_fifo_data_available_cb;
	sh2alt_ftcsr_read_delegate              m_ftcsr_read_cb;

	void sh2alt_set_frt_input(int state);
	...
	TIMER_CALLBACK_MEMBER( sh2alt_timer_callback );
	TIMER_CALLBACK_MEMBER( sh2alt_dma_current_active_callback );
	void sh2alt_timer_resync();
	void sh2alt_timer_activate();
	void sh2alt_do_dma(int dma);
	void sh2alt_exception(const char *message, int irqline);
	void sh2alt_dmac_check(int dma);
	void sh2alt_recalc_irq();
...
class sh2alta_device : public sh2alt_device
{
public:
	// construction/destruction
	sh2alta_device(const machine_config &mconfig, const char *_tag, device_t *_owner, UINT32 _clock);
...
class sh1alt_device : public sh2alt_device
{
public:
	// construction/destruction
	sh1alt_device(const machine_config &mconfig, const char *_tag, device_t *_owner, UINT32 _clock);
...
extern const device_type SH1ALT;
extern const device_type SH2ALT;
extern const device_type SH2ALTA;
