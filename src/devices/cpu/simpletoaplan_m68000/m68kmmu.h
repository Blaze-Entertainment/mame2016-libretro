...
#define SIMPLETOAPLAN_M68K_MMU_SR_BUS_ERROR 0x8000
#define SIMPLETOAPLAN_M68K_MMU_SR_SUPERVISOR_ONLY 0x2000
#define SIMPLETOAPLAN_M68K_MMU_SR_WRITE_PROTECT 0x0800
#define SIMPLETOAPLAN_M68K_MMU_SR_INVALID 0x0400
#define SIMPLETOAPLAN_M68K_MMU_SR_MODIFIED 0x0200
#define SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_0 0x0000
#define SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_1 0x0001
#define SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_2 0x0002
#define SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_3 0x0003

// MMU translation table descriptor field definitions

#define SIMPLETOAPLAN_M68K_MMU_DF_DT         0x0003
#define SIMPLETOAPLAN_M68K_MMU_DF_DT0        0x0000
#define SIMPLETOAPLAN_M68K_MMU_DF_DT1        0x0001
#define SIMPLETOAPLAN_M68K_MMU_DF_DT2        0x0002
#define SIMPLETOAPLAN_M68K_MMU_DF_DT3        0x0003
#define SIMPLETOAPLAN_M68K_MMU_DF_WP         0x0004
#define SIMPLETOAPLAN_M68K_MMU_DF_USED       0x0008
#define SIMPLETOAPLAN_M68K_MMU_DF_MODIFIED   0x0010
#define SIMPLETOAPLAN_M68K_MMU_DF_CI         0x0040
#define SIMPLETOAPLAN_M68K_MMU_DF_SUPERVISOR 0x0100

// MMU ATC Fields

#define SIMPLETOAPLAN_M68K_MMU_ATC_BUSERROR  0x08000000
#define SIMPLETOAPLAN_M68K_MMU_ATC_CACHE_IN  0x04000000
#define SIMPLETOAPLAN_M68K_MMU_ATC_WRITE_PR  0x02000000
#define SIMPLETOAPLAN_M68K_MMU_ATC_MODIFIED  0x01000000
#define SIMPLETOAPLAN_M68K_MMU_ATC_MASK      0x00ffffff
#define SIMPLETOAPLAN_M68K_MMU_ATC_SHIFT     8
#define SIMPLETOAPLAN_M68K_MMU_ATC_VALID     0x08000000

// MMU Translation Control register
#define SIMPLETOAPLAN_M68K_MMU_TC_SRE        0x02000000

/* decodes the effective address */
static UINT32 DECODE_EA_32(simpletoaplan_m68000_base_device *m68k, int ea)
{
	int mode = (ea >> 3) & 0x7;
	int reg = (ea & 0x7);
...
*/
void pmmu_atc_add(simpletoaplan_m68000_base_device *m68k, UINT32 logical, UINT32 physical, int fc)
{
	int i, found;
	...
	UINT32 atc_tag = SIMPLETOAPLAN_M68K_MMU_ATC_VALID | ((fc &7) << 24)| logical >> ps;

	// first see if this is already in the cache
	for (i = 0; i < SIMPLETOAPLAN_MMU_ATC_ENTRIES; i++)
	{
	...
	for (i = 0; i < SIMPLETOAPLAN_MMU_ATC_ENTRIES; i++)
	{
		if (!(m68k->mmu_atc_tag[i] & SIMPLETOAPLAN_M68K_MMU_ATC_VALID))
		{
			found = i;
			break;
		}
	...
	if (found == -1)
	{
		...
		if (m68k->mmu_atc_rr >= SIMPLETOAPLAN_MMU_ATC_ENTRIES)
		{
			m68k->mmu_atc_rr = 0;
		}
	}
	...
	if (m68k->mmu_tmp_sr & SIMPLETOAPLAN_M68K_MMU_SR_WRITE_PROTECT)
	{
		m68k->mmu_atc_data[found] |= SIMPLETOAPLAN_M68K_MMU_ATC_WRITE_PR;
	}
}
...
*/
void pmmu_atc_flush(simpletoaplan_m68000_base_device *m68k)
{
	int i;
...
static inline UINT32 get_dt2_table_entry(simpletoaplan_m68000_base_device *m68k, UINT32 tptr, UINT8 ptest)
{
	UINT32 tbl_entry = m68k->program->read_dword(tptr);
	UINT32 dt = tbl_entry & SIMPLETOAPLAN_M68K_MMU_DF_DT;

	m68k->mmu_tmp_sr |= tbl_entry & 0x0004 ? SIMPLETOAPLAN_M68K_MMU_SR_WRITE_PROTECT : 0;

	if (!ptest && dt != SIMPLETOAPLAN_M68K_MMU_DF_DT0)
	{
		if (dt == SIMPLETOAPLAN_M68K_MMU_DF_DT1 && !m68k->mmu_tmp_rw && !(m68k->mmu_tmp_sr & SIMPLETOAPLAN_M68K_MMU_SR_WRITE_PROTECT))
		{
			// set used and modified
			m68k->program->write_dword( tptr, tbl_entry | SIMPLETOAPLAN_M68K_MMU_DF_USED | SIMPLETOAPLAN_M68K_MMU_DF_MODIFIED);
		}
		else if (!(tbl_entry & SIMPLETOAPLAN_M68K_MMU_DF_USED))
		{
			m68k->program->write_dword( tptr, tbl_entry | SIMPLETOAPLAN_M68K_MMU_DF_USED);
		}
	}
	return tbl_entry;
}

static inline UINT32 get_dt3_table_entry(simpletoaplan_m68000_base_device *m68k, UINT32 tptr, UINT8 fc, UINT8 ptest)
{
	UINT32 tbl_entry2 = m68k->program->read_dword(tptr);
	UINT32 tbl_entry = m68k->program->read_dword(tptr + 4);
	UINT32 dt = tbl_entry2 & SIMPLETOAPLAN_M68K_MMU_DF_DT;

	m68k->mmu_tmp_sr |= ((tbl_entry2 & 0x0100) && !(fc & 4)) ? SIMPLETOAPLAN_M68K_MMU_SR_SUPERVISOR_ONLY : 0;
	m68k->mmu_tmp_sr |= tbl_entry2 & 0x0004 ? SIMPLETOAPLAN_M68K_MMU_SR_WRITE_PROTECT : 0;
	...
	if (!ptest && dt != SIMPLETOAPLAN_M68K_MMU_DF_DT0)
	{
		...
		{
			// set modified
			m68k->program->write_dword( tptr, tbl_entry2 | SIMPLETOAPLAN_M68K_MMU_DF_USED | SIMPLETOAPLAN_M68K_MMU_DF_MODIFIED);
		}
		else if (!(tbl_entry2 & SIMPLETOAPLAN_M68K_MMU_DF_USED))
		{
			m68k->program->write_dword( tptr, tbl_entry2 | SIMPLETOAPLAN_M68K_MMU_DF_USED);
		}
	}

	return (tbl_entry & ~SIMPLETOAPLAN_M68K_MMU_DF_DT) | dt;
}

/*
    pmmu_translate_addr_with_fc: perform 68851/68030-style PMMU address translation
*/
/*static inline*/ static UINT32 pmmu_translate_addr_with_fc(simpletoaplan_m68000_base_device *m68k, UINT32 addr_in, UINT8 fc, UINT8 ptest)
{
...
//  }
	...
	atc_tag = SIMPLETOAPLAN_M68K_MMU_ATC_VALID | ((fc &7) << 24) | addr_in >> ps;
	...
	for (i = 0; i < SIMPLETOAPLAN_MMU_ATC_ENTRIES; i++)
	{
		...
		else if (!m68k->mmu_tmp_rw && (m68k->mmu_atc_data[i] & SIMPLETOAPLAN_M68K_MMU_ATC_WRITE_PR))
		{
			// write mode, but write protected
		}
		else if (!m68k->mmu_tmp_rw && !(m68k->mmu_atc_data[i] & SIMPLETOAPLAN_M68K_MMU_ATC_MODIFIED))
		{
		...
		else
		{
			...
			if (!m68k->mmu_tmp_rw && !ptest)
			{
				...
				m68k->mmu_atc_data[i] |= SIMPLETOAPLAN_M68K_MMU_ATC_MODIFIED;
			}
			else
			{
				// FIXME: supervisor mode?
				m68k->mmu_tmp_sr = SIMPLETOAPLAN_M68K_MMU_SR_MODIFIED;
			}
...
//          pmmu_atc_count++;
	...
	if ((m68k->mmu_tc & SIMPLETOAPLAN_M68K_MMU_TC_SRE) && (fc & 4))
	{
	...
	switch (root_limit & SIMPLETOAPLAN_M68K_MMU_DF_DT)
	{
		case SIMPLETOAPLAN_M68K_MMU_DF_DT0:   // invalid, will cause MMU exception
			m68k->mmu_tmp_sr |= SIMPLETOAPLAN_M68K_MMU_SR_INVALID;
			return root_aptr;

		case SIMPLETOAPLAN_M68K_MMU_DF_DT1:   // page descriptor, will cause direct mapping
			addr_out = tptr + addr_in;
//          printf("PMMU: PC=%x root mode %d (addr_in %08x -> %08x)\n", m68k->ppc, SIMPLETOAPLAN_M68K_MMU_DF_DT1, addr_in, addr_out);
			return addr_out;

		case SIMPLETOAPLAN_M68K_MMU_DF_DT2:   // valid 4 byte descriptors
			tofs *= 4;
//          if (verbose) printf("PMMU: reading table A entry at %08x\n", tofs + tptr);
			tbl_entry = get_dt2_table_entry(m68k,  tptr + tofs,  ptest);
			tamode = tbl_entry & SIMPLETOAPLAN_M68K_MMU_DF_DT;
//          if (verbose) printf("PMMU: addr %08x entry %08x mode %x tofs %x\n", addr_in, tbl_entry, tamode, tofs);
			break;

		case SIMPLETOAPLAN_M68K_MMU_DF_DT3: // valid 8 byte descriptors
			tofs *= 8;
...
//          if (verbose) printf("PMMU: addr %08x entry %08x entry2 %08x mode %x tofs %x\n", addr_in, tbl_entry, tbl_entry2, tamode, tofs);
	...
	switch (tamode)
	{
		case SIMPLETOAPLAN_M68K_MMU_DF_DT0: // invalid, will cause MMU exception (but not for ptest)
			m68k->mmu_tmp_sr |= (SIMPLETOAPLAN_M68K_MMU_SR_INVALID | SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_1);
			// last valid pointer (for ptest)
			addr_out = last_entry_ptr;
			resolved = 1;
			break;

		case SIMPLETOAPLAN_M68K_MMU_DF_DT2: // 4-byte table B descriptor
			tofs *= 4;
//          if (verbose) printf("PMMU: reading table B entry at %08x\n", tofs + tptr);
			tbl_entry = get_dt2_table_entry(m68k, tptr + tofs,  ptest);
			tbmode = tbl_entry & SIMPLETOAPLAN_M68K_MMU_DF_DT;
//          if (verbose) printf("PMMU: addr %08x entry %08x mode %x tofs %x\n", addr_in, tbl_entry, tbmode, tofs);
			break;

		case SIMPLETOAPLAN_M68K_MMU_DF_DT3: // 8-byte table B descriptor
			tofs *= 8;
//          if (verbose) printf("PMMU: reading table B entries at %08x\n", tofs + tptr);
			tbl_entry = get_dt3_table_entry(m68k, tptr + tofs, fc,  ptest);
			tbmode = tbl_entry & SIMPLETOAPLAN_M68K_MMU_DF_DT;
			tbl_entry &= ~SIMPLETOAPLAN_M68K_MMU_DF_DT;
//          if (verbose) printf("PMMU: addr %08x entry %08x entry2 %08x mode %x tofs %x\n", addr_in, tbl_entry, tbl_entry2, tbmode, tofs);
			break;

		case SIMPLETOAPLAN_M68K_MMU_DF_DT1:   // early termination descriptor
			tbl_entry &= (~0 << ps);
	...
	if (!resolved)
	{
		...
		{
			case SIMPLETOAPLAN_M68K_MMU_DF_DT0:   // invalid, will cause MMU exception (but not for ptest)
				m68k->mmu_tmp_sr |= (SIMPLETOAPLAN_M68K_MMU_SR_INVALID | SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_2);
				// last valid pointer (for ptest)
				addr_out = last_entry_ptr;
				resolved = 1;
				break;

			case SIMPLETOAPLAN_M68K_MMU_DF_DT2: // 4-byte table C descriptor
				tofs *= 4;
//              if (verbose) printf("PMMU: reading table C entry at %08x\n", tofs + tptr);
				tbl_entry = get_dt2_table_entry(m68k, tptr + tofs, ptest);
				tcmode = tbl_entry & SIMPLETOAPLAN_M68K_MMU_DF_DT;
//              if (verbose) printf("PMMU: addr %08x entry %08x mode %x tofs %x\n", addr_in, tbl_entry, tbmode, tofs);
				break;

			case SIMPLETOAPLAN_M68K_MMU_DF_DT3: // 8-byte table C descriptor
				tofs *= 8;
...
//              if (verbose) printf("PMMU: addr %08x entry %08x entry2 %08x mode %x tofs %x\n", addr_in, tbl_entry, tbl_entry2, tcmode, tofs);
			...
			case SIMPLETOAPLAN_M68K_MMU_DF_DT1: // termination descriptor
				tbl_entry &= (~0 << ps);
	...
	if (!resolved)
	{
		switch (tcmode)
		{
			case SIMPLETOAPLAN_M68K_MMU_DF_DT0:   // invalid, will cause MMU exception (unless ptest)
				m68k->mmu_tmp_sr |= (SIMPLETOAPLAN_M68K_MMU_SR_INVALID | SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_3);
				addr_out = tptr + tofs;
				resolved = 1;
				break;

			case SIMPLETOAPLAN_M68K_MMU_DF_DT2: // 4-byte (short-form) indirect descriptor
			case SIMPLETOAPLAN_M68K_MMU_DF_DT3: // 8-byte (long-form) indirect descriptor
	...
	if (!ptest)
	{
		if (m68k->mmu_tmp_sr & SIMPLETOAPLAN_M68K_MMU_SR_INVALID)
		{
		...
		else if (m68k->mmu_tmp_sr & SIMPLETOAPLAN_M68K_MMU_SR_SUPERVISOR_ONLY)
		{
			if (++m68k->mmu_tmp_buserror_occurred == 1)
			{
				m68k->mmu_tmp_buserror_address = addr_in;
		...
		else if ((m68k->mmu_tmp_sr & SIMPLETOAPLAN_M68K_MMU_SR_WRITE_PROTECT) && !m68k->mmu_tmp_rw)
		{
			if (++m68k->mmu_tmp_buserror_occurred == 1)
			{
				m68k->mmu_tmp_buserror_address = addr_in;
...
/*static inline*/ static UINT32 pmmu_translate_addr_with_fc_040(simpletoaplan_m68000_base_device *m68k, UINT32 addr_in, UINT8 fc, UINT8 ptest)
{
...
*/
/*static inline*/ static UINT32 pmmu_translate_addr(simpletoaplan_m68000_base_device *m68k, UINT32 addr_in)
{
	UINT32 addr_out;
...
//              REG_PPC(m68k), REG_A(m68k)[7], addr_in, addr_out, m68k->mmu_tmp_sr & SIMPLETOAPLAN_M68K_MMU_SR_LEVEL_3,
...
void m68881_mmu_ops(simpletoaplan_m68000_base_device *m68k)
{
	UINT16 modes;
...
static inline UINT32 hmmu_translate_addr(simpletoaplan_m68000_base_device *m68k, UINT32 addr_in)
{
	UINT32 addr_out;
	...
	if (m68k->hmmu_enabled == SIMPLETOAPLAN_M68K_HMMU_ENABLE_LC)
	{
		addr_out = addr_in & 0xffffff;
	}
	else if (m68k->hmmu_enabled == SIMPLETOAPLAN_M68K_HMMU_ENABLE_II) // the original II does a more complex translation
	{
		addr_out = addr_in & 0xffffff;
