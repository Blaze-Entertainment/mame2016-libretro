...
extern void (*m68ki_instruction_jump_table[][0x10000])(simpletoaplan_m68000_base_device *m68k); /* opcode handler jump table */
...
void (*m68ki_instruction_jump_table[NUM_CPU_TYPES][0x10000])(simpletoaplan_m68000_base_device *m68k); /* opcode handler jump table */
unsigned char m68ki_cycles[NUM_CPU_TYPES][0x10000]; /* Cycles used by CPU type */

/* This is used to generate the opcode handler jump table */
struct opcode_handler_struct
{
	void (*opcode_handler)(simpletoaplan_m68000_base_device *m68k);        /* handler function */
...
void m68ki_build_opcode_table(void)
{
	...
	for(i = 0; i < 0x10000; i++)
	{
		...
		{
			m68ki_instruction_jump_table[k][i] = simpletoaplan_m68000_base_device_ops::m68k_op_illegal;
			m68ki_cycles[k][i] = 0;
		}
	}
...
extern void m68881_ftrap(simpletoaplan_m68000_base_device *m68k);
...
M68KMAKE_OP(1010, 0, ., .)
{
	simpletoaplan_m68ki_exception_1010(mc68kcpu);
}


M68KMAKE_OP(1111, 0, ., .)
{
	simpletoaplan_m68ki_exception_1111(mc68kcpu);
}
...
M68KMAKE_OP(andi, 16, tos, .)
{
	...
	simpletoaplan_m68ki_exception_privilege_violation(mc68kcpu);
}
...
M68KMAKE_OP(asr, 8, s, .)
{
	...
	if(GET_MSB_8(src))
		res |= simpletoaplan_m68ki_shift_8_table[shift];
...
M68KMAKE_OP(asr, 16, s, .)
{
	...
	if(GET_MSB_16(src))
		res |= simpletoaplan_m68ki_shift_16_table[shift];
...
M68KMAKE_OP(asr, 32, s, .)
{
	...
	if(GET_MSB_32(src))
		res |= simpletoaplan_m68ki_shift_32_table[shift];
...
M68KMAKE_OP(asl, 8, s, .)
{
	...
	*r_dst = MASK_OUT_BELOW_8(*r_dst) | res;
	...
	(mc68kcpu)->n_flag = NFLAG_8(res);
	(mc68kcpu)->not_z_flag = res;
	src &= simpletoaplan_m68ki_shift_8_table[shift + 1];
	(mc68kcpu)->v_flag = (!(src == 0 || (src == simpletoaplan_m68ki_shift_8_table[shift + 1] && shift < 8)))<<7;
}
...
M68KMAKE_OP(asl, 16, s, .)
{
	...
	*r_dst = MASK_OUT_BELOW_16(*r_dst) | res;
	...
	src &= simpletoaplan_m68ki_shift_16_table[shift + 1];
	(mc68kcpu)->v_flag = (!(src == 0 || src == simpletoaplan_m68ki_shift_16_table[shift + 1]))<<7;
}
...
M68KMAKE_OP(asl, 32, s, .)
{
	...
	*r_dst = res;
	...
	src &= simpletoaplan_m68ki_shift_32_table[shift + 1];
	(mc68kcpu)->v_flag = (!(src == 0 || src == simpletoaplan_m68ki_shift_32_table[shift + 1]))<<7;
}
	...
	if(shift != 0)
	{
		...
		if(shift < 8)
		{
			*r_dst = MASK_OUT_BELOW_8(*r_dst) | res;
			...
			(mc68kcpu)->not_z_flag = res;
			src &= simpletoaplan_m68ki_shift_8_table[shift + 1];
			(mc68kcpu)->v_flag = (!(src == 0 || src == simpletoaplan_m68ki_shift_8_table[shift + 1]))<<7;
			return;
		}
...
M68KMAKE_OP(bfchg, 32, ., d)
{
	...
	simpletoaplan_m68ki_exception_illegal(mc68kcpu);
}
...
M68KMAKE_OP(chk, 16, ., d)
{
	...
	simpletoaplan_m68ki_exception_trap((mc68kcpu), EXCEPTION_CHK);
}
...
M68KMAKE_OP(divs, 16, ., d)
{
	...
	simpletoaplan_m68ki_exception_trap((mc68kcpu), EXCEPTION_ZERO_DIVIDE);
}
...
//                  mc68kcpu->logerror("movec to cacr=%04x\n", (mc68kcpu)->cacr);
					if ((mc68kcpu)->cacr & (SIMPLETOAPLAN_M68K_CACR_CI | SIMPLETOAPLAN_M68K_CACR_CEI))
					{
...
M68KMAKE_OP(rte, 32, ., .)
{
	if((mc68kcpu)->s_flag)
	{
		...
		if(CPU_TYPE_IS_010((mc68kcpu)->cpu_type))
		{
			...
			simpletoaplan_m68ki_exception_format_error(mc68kcpu);
			return;
		}
...
//  (mc68kcpu)->v_flag = ~res; /* Undefined V behavior */
	(mc68kcpu)->v_flag = VFLAG_CLEAR;   /* Undefined in Motorola's SIMPLETOAPLAN_M68000PM/AD rev.1 and safer to assume cleared. */
	...
	if(res > 0x99)
	{
		...
		(mc68kcpu)->n_flag = NFLAG_SET; /* Undefined in Motorola's SIMPLETOAPLAN_M68000PM/AD rev.1 and safer to follow carry. */
	}
	else
...
//  (mc68kcpu)->v_flag = ~res; /* Undefined V behavior */
	(mc68kcpu)->v_flag = VFLAG_CLEAR;   /* Undefined in Motorola's SIMPLETOAPLAN_M68000PM/AD rev.1 and safer to return zero. */
...
M68KMAKE_OP(trap, 0, ., .)
{
	...
	simpletoaplan_m68ki_exception_trapN((mc68kcpu), EXCEPTION_TRAP_BASE + ((mc68kcpu)->ir & 0xf));    /* HJB 990403 */
}
...
M68KMAKE_OP(trapt, 0, ., .)
{
	if(CPU_TYPE_IS_EC020_PLUS((mc68kcpu)->cpu_type))
	{
		simpletoaplan_m68ki_exception_trap((mc68kcpu), EXCEPTION_TRAPV);  /* HJB 990403 */
		return;
	}
	simpletoaplan_m68ki_exception_illegal(mc68kcpu);
}
