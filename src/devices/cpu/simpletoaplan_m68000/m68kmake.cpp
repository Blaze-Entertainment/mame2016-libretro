...
#define SIMPLETOAPLAN_M68K_MAX_PATH 1024
#define SIMPLETOAPLAN_M68K_MAX_DIR  1024
...
enum
{
	...
	CPU_TYPE_030,       // 3
	CPU_TYPE_040,       // 4
	CPU_TYPE_SIMPLETOAPLAN_FSCPU32,     // 5
	CPU_TYPE_COLDFIRE,  // 6
	NUM_CPUS
};
...
static char g_input_filename[SIMPLETOAPLAN_M68K_MAX_PATH];
...
static void write_function_name(FILE* filep, char* base_name)
{
	fprintf(filep, "void simpletoaplan_m68000_base_device_ops::%s(simpletoaplan_m68000_base_device* mc68kcpu)\n", base_name);
	fprintf(g_prototype_file, "static void %s(simpletoaplan_m68000_base_device* mc68kcpu);\n", base_name);
}
...
static void add_opcode_output_table_entry(opcode_struct* op, char* name)
{
	...
	*ptr = *op;
	...
	sprintf( ptr->name, "simpletoaplan_m68000_base_device_ops::%s", name);
...
{
	/* File stuff */
	char output_path[SIMPLETOAPLAN_M68K_MAX_DIR] = "";
	char filename[SIMPLETOAPLAN_M68K_MAX_PATH];
	/* Section identifier */
