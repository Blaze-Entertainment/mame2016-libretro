...
#define OP(num,func_name) void simpletoaplan_nec_common_device::func_name()
...
OP( 0x0f, i_pre_nec  ) { UINT32 ModRM, tmp, tmp2;
	switch (FETCH()) {
		...
		case 0xe0 : ModRM = FETCH(); ModRM=0; logerror("%06x: SIMPLETOAPLAN_V33 unimplemented BRKXA (break to expansion address)\n",PC()); break;
		case 0xf0 : ModRM = FETCH(); ModRM=0; logerror("%06x: SIMPLETOAPLAN_V33 unimplemented RETXA (return from expansion address)\n",PC()); break;
		case 0xff : ModRM = FETCH(); ModRM=0; logerror("%06x: unimplemented BRKEM (break to 8080 emulation mode)\n",PC()); break;
		default:    logerror("%06x: Unknown SIMPLETOAPLAN_V20 instruction\n",PC()); break;
	}
}
...
OP( 0x26, i_es       ) { m_seg_prefix=TRUE;    m_prefix_base=Sreg(DS1)<<4;    CLK(2);     (this->*s_simpletoaplan_nec_instruction[fetchop()])(); m_seg_prefix=FALSE; }
...
OP( 0x2e, i_cs       ) { m_seg_prefix=TRUE;    m_prefix_base=Sreg(PS)<<4; CLK(2);     (this->*s_simpletoaplan_nec_instruction[fetchop()])(); m_seg_prefix=FALSE; }
...
OP( 0x36, i_ss       ) { m_seg_prefix=TRUE;    m_prefix_base=Sreg(SS)<<4; CLK(2);     (this->*s_simpletoaplan_nec_instruction[fetchop()])(); m_seg_prefix=FALSE; }
...
OP( 0x3e, i_ds       ) { m_seg_prefix=TRUE;    m_prefix_base=Sreg(DS0)<<4;    CLK(2);     (this->*s_simpletoaplan_nec_instruction[fetchop()])(); m_seg_prefix=FALSE; }
...
static unsigned simpletoaplan_nec_popa_tmp;
OP( 0x61, i_popa  ) {
	POP(Wreg(IY));
	POP(Wreg(IX));
	POP(Wreg(BP));
	POP(simpletoaplan_nec_popa_tmp);
	POP(Wreg(BW));
	POP(Wreg(DW));
	POP(Wreg(CW));
	POP(Wreg(AW));
...
OP( 0x62, i_chkind  ) {
	...
	tmp= RegWord(ModRM);
	if (tmp<low || tmp>high) {
		simpletoaplan_nec_interrupt(SIMPLETOAPLAN_NEC_CHKIND_VECTOR, BRK);
	}
	m_icount-=20;
...
OP( 0x64, i_repnc  ) {  UINT32 next = fetchop();   UINT16 c = Wreg(CW);
	...
	switch(next) {
		...
		default:    logerror("%06x: REPNC invalid\n",PC());    (this->*s_simpletoaplan_nec_instruction[next])();
	}
	m_seg_prefix=FALSE;
}
...
OP( 0x9d, i_popf      ) { UINT32 tmp; POP(tmp); ExpandFlags(tmp); CLKS(12,8,5); if (m_TF) simpletoaplan_nec_trap(); }
...
OP( 0xcc, i_int3      ) { simpletoaplan_nec_interrupt(3, BRK); CLKS(50,50,24); }
OP( 0xcd, i_int       ) { simpletoaplan_nec_interrupt(FETCH(), BRK); CLKS(50,50,24); }
OP( 0xce, i_into      ) { if (OF) { simpletoaplan_nec_interrupt(SIMPLETOAPLAN_NEC_BRKV_VECTOR, BRK); CLKS(52,52,26); } else CLK(3); }
...
OP( 0xf6, i_f6pre ) { UINT32 tmp; UINT32 uresult,uresult2; INT32 result,result2;
	...
	switch (ModRM & 0x38) {
		...
		case 0x30: if (tmp) { DIVUB; } else simpletoaplan_nec_interrupt(SIMPLETOAPLAN_NEC_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
		case 0x38: if (tmp) { DIVB;  } else simpletoaplan_nec_interrupt(SIMPLETOAPLAN_NEC_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
	}
}
...
OP( 0xf7, i_f7pre   ) { UINT32 tmp,tmp2; UINT32 uresult,uresult2; INT32 result,result2;
	...
	switch (ModRM & 0x38) {
		...
		case 0x30: if (tmp) { DIVUW; } else simpletoaplan_nec_interrupt(SIMPLETOAPLAN_NEC_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
		case 0x38: if (tmp) { DIVW;  } else simpletoaplan_nec_interrupt(SIMPLETOAPLAN_NEC_DIVIDE_VECTOR, BRK); m_icount-=(ModRM >=0xc0 )?43:53; break;
	}
}
...
void simpletoaplan_nec_common_device::i_invalid()
{
	m_icount-=10;
