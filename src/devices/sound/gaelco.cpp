	...
	for(j = 0; j < samples; j++)
	{
		...
		for (ch = 0; ch < GAELCO_NUM_CHANNELS; ch ++)
		{
			...
			if (channel->active == 1)
			{
				...
				{
					//printf("(GAE1) Playing unknown sample format in channel: %02d, type: %02x, bank: %02x, end: %08x, Length: %04x\n", ch, type, bank, end_pos, m_sndregs[base_offset + 3]);
					//channel->active = 0;
				...
				if (m_sndregs[base_offset + 3] == 0)
				{
					...
					{
						channel->active = 0;
						//printf("%02d --------------------------------------------- Channel Stopped after Sample: type: %02x, bank: %02x, end: %08x, Length: %04x\n", ch, type, bank, end_pos, m_sndregs[base_offset + 3]);
					}
					else                    /* if we're looping, swap chunks */
					{
						...
						{
							//printf("%02d --------------------------------------------- Stopped due to no loop data\n", ch);
							channel->active = 0;
						}
						else
						{
							//printf("%02d ++++++++++++++++++++++++++ Channel Looping after Sample: type: %02x, bank: %02x, end: %08x, Length: %04x\n", ch, type, bank, end_pos, m_sndregs[base_offset + 3]);
						}
...
void gaelco_gae1_device::device_start()
{
	...
	for (int i = 0; i < GAELCO_NUM_CHANNELS; i++)
	{
		save_item(NAME(m_channel[i].active), i);
		save_item(NAME(m_channel[i].loop), i);
		save_item(NAME(m_channel[i].chunkNum), i);
	}
...
{
	for (int ch = 0; ch < GAELCO_NUM_CHANNELS; ch++)
	{
