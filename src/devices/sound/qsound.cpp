...
void qsound_device::init_register_map()
{
	// unused registers
...
int16_t qsound_device::read_sample(uint16_t bank, uint16_t address)
{
	bank &= 0x7FFF;
...
void qsound_device::update_sample()
{
	switch (m_state)
	{
		default:
...
void qsound_device::state_refresh_filter_1()
{
...
void qsound_device::state_refresh_filter_2()
{
	for (int ch = 0; ch < 2; ch++)
	{
		m_filter[ch].m_delay_pos = 0;
...
int16_t qsound_device::qsound_voice::update(qsound_device &dsp, int32_t *echo_out)
{
...
// emulators.
int16_t qsound_device::qsound_adpcm::update(qsound_device &dsp, int16_t curr_sample, int nibble)
{
	int8_t step;
	if (!nibble)
	{
...
int16_t qsound_device::qsound_echo::apply(int32_t input)
{
...
void qsound_device::state_normal_update()
{
	m_ready_flag = 0x80;
...
int32_t qsound_device::qsound_fir::apply(int16_t input)
{
...
int32_t qsound_device::qsound_delay::apply(const int32_t input)
{
...
void qsound_device::qsound_delay::update()
{
