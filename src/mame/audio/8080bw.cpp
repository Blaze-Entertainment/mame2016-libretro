...
// 09 invader hit
// 0a bonus life
// 0b not a standard invader sound

WRITE8_MEMBER(_8080bw_state::invaders_hack_audio_1_w)
{
	UINT8 rising_bits = data & ~m_port_1_last_extra;

	m_sn->enable_w((~data >> 0) & 0x01);    /* saucer sound */

//	m_discrete->write(space, INVADERS_NODE(INVADERS_MISSILE_EN, 1), data & 0x02); /* MISSLE SOUND */

	if (rising_bits & 0x02) /* MISSLE SOUND */
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x02);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}
	...
	m_discrete->write(space, INVADERS_NODE(INVADERS_EXPLOSION_EN, 1), data & 0x04);
	/*
	if (rising_bits & 0x04) // INVADERS_EXPLOSION_EN
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x03);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}
...
//	m_discrete->write(space, INVADERS_NODE(INVADERS_INVADER_HIT_EN, 1), data & 0x08);
	...
	if (rising_bits & 0x08) /* INVADERS_INVADER_HIT_EN */
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x09);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}
...
//	m_discrete->write(space, INVADERS_NODE(INVADERS_BONUS_MISSLE_BASE_EN, 1), data & 0x10);
	...
	if (rising_bits & 0x10) /* INVADERS_BONUS_MISSLE_BASE_EN */
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x0a);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}
...
WRITE8_MEMBER(_8080bw_state::invaders_hack_audio_2_w)
{
	UINT8 rising_bits = data & ~m_port_3_last_extra;

//	m_discrete->write(space, INVADERS_NODE(INVADERS_FLEET_DATA, 1), data & 0x0f);

	if (rising_bits & 0x01) /* INVADERS_FLEET_DATA */
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x04);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}

	if (rising_bits & 0x02) /* INVADERS_FLEET_DATA */
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x05);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}

	if (rising_bits & 0x04) /* INVADERS_FLEET_DATA */
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x06);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}

	if (rising_bits & 0x08) /* INVADERS_FLEET_DATA */
	{
		if (m_tsys)
		{
			m_tsys->master_port_w(space, 0, 0x00);
			m_tsys->master_comm_w(space, 0, 0x07);
			m_tsys->master_port_w(space, 0, 0x01);
			m_tsys->master_comm_w(space, 0, 0x00);
		}
	}
