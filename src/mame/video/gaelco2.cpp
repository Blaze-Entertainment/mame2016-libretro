...
{
	const uint16_t data = m_videoram[(((m_vregs[Layer] >> 9) & 0x07) * 0x2000 / 2) + (tile_index << 1)];
	const uint16_t data2 = m_videoram[(((m_vregs[Layer] >> 9) & 0x07) * 0x2000 / 2) + ((tile_index << 1) + 1)];
	const uint32_t code = ((data & 0x07) << 16) | (data2 & 0xffff);
...
{
	const uint32_t pant0_start = ((m_vregs[0] >> 9) & 0x07) * 0x1000;
	const uint32_t pant0_end = pant0_start + 0x1000;
	const uint32_t pant1_start = ((m_vregs[1] >> 9) & 0x07) * 0x1000;
	const uint32_t pant1_end = pant1_start + 0x1000;
...
static constexpr uint8_t RGB_CHG = 0x08;
static inline const uint8_t ADJUST_COLOR(int16_t c) { return (c < 0) ? 0 : ((c > 255) ? 255 : c); }

/* table used for color adjustment */
static const int8_t pen_color_adjust[16] = {
...
WRITE16_MEMBER(gaelco2_state::gaelco2_palette_w)
{
	...
	for (int i = 1; i < 16; i++)
	{
		const uint8_t auxr = ADJUST_COLOR(r + pen_color_adjust[i]);
		const uint8_t auxg = ADJUST_COLOR(g + pen_color_adjust[i]);
		const uint8_t auxb = ADJUST_COLOR(b + pen_color_adjust[i]);
...
void gaelco2_state::draw_sprites(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect, int mask)
{
	...
	const uint32_t start_offset = (m_vregs[1] & 0x10) * 0x100;
	const uint32_t end_offset = start_offset + 0x1000;
	...
	{
		const uint16_t data = buffered_spriteram16[(j / 2) + 0];
		const uint16_t data2 = buffered_spriteram16[(j / 2) + 1];
		const uint16_t data3 = buffered_spriteram16[(j / 2) + 2];
		const uint16_t data4 = buffered_spriteram16[(j / 2) + 3];
		...
		if ((data2 & 0x0200) != 0)
		{
			for (int y = 0; y < ysize; y++)
			{
				for (int x = 0; x < xsize; x++)
				{
					...
					const uint16_t data5 = buffered_spriteram16[((data4 / 2) + (y*xsize + x)) & 0x7fff];
					const uint32_t number = ((data & 0x1ff) << 10) + (data5 & 0x0fff);
					const uint32_t color = ((data >> 9) & 0x7f) + ((data5 >> 12) & 0x0f);
					...
					{ /* last palette entry is reserved for shadows and highlights */
						...
						for (int py = 0; py < gfx->height(); py++)
						{
							...
							uint16_t *const srcy = &bitmap.pix(ypos);
							...
							for (int px = 0; px < gfx->width(); px++)
							{
								...
								const uint8_t gfx_pen = gfx_src[gfx->rowbytes() * gfx_py + gfx_px];

								if ((gfx_pen == 0) || (gfx_pen >= 16)) continue;

								uint16_t *const pixel = srcy + xpos;
								const uint16_t src_color = *pixel;
