...
(move below MODE notes to upd7810.cpp?)
...
extern const device_type TAITO_CCHIP = &device_creator<taito_cchip_device>;


taito_cchip_device::taito_cchip_device(const machine_config &mconfig, const char *tag, device_t *owner, UINT32 clock)
	: device_t(mconfig, TAITO_CCHIP, "Taito C-Chip", tag, owner, clock, "cchip", __FILE__),
	m_upd7810(*this, "upd7810"),
...
{
	m_upd7810->set_input_line(UPD7810_INTF1, state);
}

ROM_START( taito_cchip )
	ROM_REGION( 0x1000, "upd7810", 0 )
...
static ADDRESS_MAP_START(cchip_map, AS_PROGRAM, 8, taito_cchip_device)
    ...
    AM_RANGE(0x1000, 0x13ff) AM_READWRITE(mem_r, mem_w);
    AM_RANGE(0x1400, 0x17ff) AM_READWRITE(asic_r, asic_w);
	AM_RANGE(0x2000, 0x3fff) AM_ROM AM_REGION("^cchip_eprom", 0x0000)
    AM_RANGE(0xff00, 0xffff) AM_RAM  // internal RAM of uPD7811
ADDRESS_MAP_END

static ADDRESS_MAP_START(cchip_io_map, AS_IO, 8, taito_cchip_device)
    AM_RANGE(0x00, 0x00) AM_READWRITE(pa_r, pa_w);
    AM_RANGE(0x01, 0x01) AM_READWRITE(pb_r, pb_w);
    AM_RANGE(0x02, 0x02) AM_READWRITE(pc_r, pc_w);
    AM_RANGE(0x04, 0x04) AM_WRITE(pf_w);
ADDRESS_MAP_END


READ_LINE_MEMBER( taito_cchip_device::an0_r ){ return BIT(m_in_ad_cb(), 0) ? 0xff : 0; }
READ_LINE_MEMBER( taito_cchip_device::an1_r ){ return BIT(m_in_ad_cb(), 1) ? 0xff : 0; }
READ_LINE_MEMBER( taito_cchip_device::an2_r ){ return BIT(m_in_ad_cb(), 2) ? 0xff : 0; }
READ_LINE_MEMBER( taito_cchip_device::an3_r ){ return BIT(m_in_ad_cb(), 3) ? 0xff : 0; }
READ_LINE_MEMBER( taito_cchip_device::an4_r ){ return BIT(m_in_ad_cb(), 4) ? 0xff : 0; }
READ_LINE_MEMBER( taito_cchip_device::an5_r ){ return BIT(m_in_ad_cb(), 5) ? 0xff : 0; }
READ_LINE_MEMBER( taito_cchip_device::an6_r ){ return BIT(m_in_ad_cb(), 6) ? 0xff : 0; }
READ_LINE_MEMBER( taito_cchip_device::an7_r ){ return BIT(m_in_ad_cb(), 7) ? 0xff : 0; }


READ8_MEMBER( taito_cchip_device::pa_r ){ return m_in_pa_cb(); }
READ8_MEMBER( taito_cchip_device::pb_r ){ return m_in_pb_cb(); }
READ8_MEMBER( taito_cchip_device::pc_r ){ return m_in_pc_cb(); }

WRITE8_MEMBER( taito_cchip_device::pa_w ){ m_out_pa_cb(data); }
WRITE8_MEMBER( taito_cchip_device::pb_w ){ m_out_pb_cb(data); }
WRITE8_MEMBER( taito_cchip_device::pc_w ){ m_out_pc_cb(data); }
WRITE8_MEMBER( taito_cchip_device::pf_w ){ /*logerror("%s port F written %.2x\n", machine().describe_context(), data);*/ }

static MACHINE_CONFIG_FRAGMENT(taitocchip)
    MCFG_CPU_ADD("upd7810", UPD7810, 12000000)
...
machine_config_constructor taito_cchip_device::device_mconfig_additions() const
{
	return MACHINE_CONFIG_NAME( taitocchip );
}
...
void taito_cchip_device::device_start()
{
	...
	save_item(NAME(m_sharedram));

    m_upd4464_bank = 0;
    m_upd4464_bank68 = 0;

   	save_item(NAME(m_upd4464_bank));
	save_item(NAME(m_upd4464_bank68));
...
const rom_entry *taito_cchip_device::device_rom_region() const
{
