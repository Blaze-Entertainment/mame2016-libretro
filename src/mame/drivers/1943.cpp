...
{
	32,32,  /* 32*32 tiles */
	RGN_FRAC(1,1),    /* 512 tiles */
	4,      /* 4 bits per pixel */
	{ 12, 8, 4, 0 },
	{ 0, 1, 2, 3, 16, 17, 18, 19,
			128*8+0, 128*8+1, 128*8+2, 128*8+3, 128*8+16, 128*8+17, 128*8+18, 128*8+19,
			256*8+0, 256*8+1, 256*8+2, 256*8+3, 256*8+16, 256*8+17, 256*8+18, 256*8+19,
			384*8+0, 384*8+1, 384*8+2, 384*8+3, 384*8+16, 384*8+17, 384*8+18, 384*8+19 },
	{ 0*32, 1*32, 2*32, 3*32, 4*32, 5*32, 6*32, 7*32,
			8*32, 9*32, 10*32, 11*32, 12*32, 13*32, 14*32, 15*32,
			16*32, 17*32, 18*32, 19*32, 20*32, 21*32, 22*32, 23*32,
			24*32, 25*32, 26*32, 27*32, 28*32, 29*32, 30*32, 31*32 },
	512*8   /* every tile takes 256 consecutive bytes */
};
...
ROM_START( 1943 )
	...
	ROM_LOAD( "bme01.12d", 0x00000, 0x08000, CRC(55fd447e) SHA1(f9125745ce85282aa487f744cbf509f335dc3e85) ) /* This rom was hacked, we need a dump of an original */
	...
	ROM_LOAD16_BYTE( "bm15.10f", 0x00000, 0x8000, CRC(6b1a0443) SHA1(32337c840ccd6815fd5844c194365c58d708f6dc) )   /* bg tiles */
	ROM_LOAD16_BYTE( "bm16.11f", 0x10000, 0x8000, CRC(23c908c2) SHA1(42b83ff5781be9181802a21ff1b23c17ab1bc5a2) )
	ROM_LOAD16_BYTE( "bm17.12f", 0x20000, 0x8000, CRC(46bcdd07) SHA1(38feda668be25d1adc04aa36afc73b07c1545f89) )
	ROM_LOAD16_BYTE( "bm18.14f", 0x30000, 0x8000, CRC(e6ae7ba0) SHA1(959c306dc28b9be2adc54b3d46312d26764c7b8b) )
	
	ROM_LOAD16_BYTE( "bm19.10j", 0x00001, 0x8000, CRC(868ababc) SHA1(1c7be905f53c63bad25fbbd9b3cf82d2c7749bc3) )
	ROM_LOAD16_BYTE( "bm20.11j", 0x10001, 0x8000, CRC(0917e5d4) SHA1(62dd277bc1fa54cfe168ae2380bc147bd17f4205) )
	ROM_LOAD16_BYTE( "bm21.12j", 0x20001, 0x8000, CRC(9bfb0d89) SHA1(f1bae7ec46edcf46c7af84c054e89b322f8c8972) )
	ROM_LOAD16_BYTE( "bm22.14j", 0x30001, 0x8000, CRC(04f3c274) SHA1(932780c04abe285e1ec67b726b145175f73eafe0) )

	ROM_REGION( 0x10000, "gfx3", 0 )
	ROM_LOAD16_BYTE( "bm24.14k", 0x00000, 0x8000, CRC(11134036) SHA1(88da112ab9fc7e0d8f0e901f273715b950ae588c) )   /* fg tiles */
	ROM_LOAD16_BYTE( "bm25.14l", 0x00001, 0x8000, CRC(092cf9c1) SHA1(19fe3c714b1d52cbb21dea25cdee5af841f525db) )
...
void _1943_state::show_tile_data(int tile)
{
	int height = 32;
	int width = 32;


	int tilesize = (4 * width * height) / 8;
	int tilebase = tile * tilesize;

	printf("uint8_t show_tile_data_%04x[32 * 32] = { \n", tile);
	...
	for (int y = 0; y < height; y++)
	{
		...
		{
			int lineaddr = tilebase + y * (4);
			lineaddr += x * 128;

			uint8_t pix[8];

			pix[0] = ((rom[lineaddr+0] & 0x80) >> 7) | ((rom[lineaddr+0] & 0x08) >> 2) | ((rom[lineaddr+1] & 0x80) >> 5) | ((rom[lineaddr+1] & 0x08) >> 0);
			pix[1] = ((rom[lineaddr+0] & 0x40) >> 6) | ((rom[lineaddr+0] & 0x04) >> 1) | ((rom[lineaddr+1] & 0x40) >> 4) | ((rom[lineaddr+1] & 0x04) << 1);
			pix[2] = ((rom[lineaddr+0] & 0x20) >> 5) | ((rom[lineaddr+0] & 0x02) >> 0) | ((rom[lineaddr+1] & 0x20) >> 3) | ((rom[lineaddr+1] & 0x02) << 2);
			pix[3] = ((rom[lineaddr+0] & 0x10) >> 4) | ((rom[lineaddr+0] & 0x01) << 1) | ((rom[lineaddr+1] & 0x10) >> 2) | ((rom[lineaddr+1] & 0x01) << 3);
			
			pix[4] = ((rom[lineaddr+2] & 0x80) >> 7) | ((rom[lineaddr+2] & 0x08) >> 2) | ((rom[lineaddr+3] & 0x80) >> 5) | ((rom[lineaddr+3] & 0x08) >> 0);
			pix[5] = ((rom[lineaddr+2] & 0x40) >> 6) | ((rom[lineaddr+2] & 0x04) >> 1) | ((rom[lineaddr+3] & 0x40) >> 4) | ((rom[lineaddr+3] & 0x04) << 1);
			pix[6] = ((rom[lineaddr+2] & 0x20) >> 5) | ((rom[lineaddr+2] & 0x02) >> 0) | ((rom[lineaddr+3] & 0x20) >> 3) | ((rom[lineaddr+3] & 0x02) << 2);
			pix[7] = ((rom[lineaddr+2] & 0x10) >> 4) | ((rom[lineaddr+2] & 0x01) << 1) | ((rom[lineaddr+3] & 0x10) >> 2) | ((rom[lineaddr+3] & 0x01) << 3);
			...
			//printf("addr %08x\n", addr);
			rom[lineaddr+0] = machine().rand();
			rom[lineaddr+1] = machine().rand();
			rom[lineaddr+2] = machine().rand();
			rom[lineaddr+3] = machine().rand();
		}
...
void _1943_state::put_tile_data(int tile, uint8_t* data)
{
	int height = 32;
	int width = 32;
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width / 8; x++)
		{
			...
			uint8_t* pix = (data + y * width) + x * 8;

			rom[lineaddr + 0] = ((pix[0] & 0x01) << 7) | ((pix[1] & 0x01) << 6) | ((pix[2] & 0x01) << 5) | ((pix[3] & 0x01) << 4) | ((pix[0] & 0x02) << 2) | ((pix[1] & 0x02) << 1) | ((pix[2] & 0x02) << 0) | ((pix[3] & 0x02) >> 1);
			rom[lineaddr + 1] = ((pix[0] & 0x04) << 5) | ((pix[1] & 0x04) << 4) | ((pix[2] & 0x04) << 3) | ((pix[3] & 0x04) << 2) | ((pix[0] & 0x08) >> 0) | ((pix[1] & 0x08) >> 1) | ((pix[2] & 0x08) >> 2) | ((pix[3] & 0x08) >> 3);

			rom[lineaddr + 2] = ((pix[4] & 0x01) << 7) | ((pix[5] & 0x01) << 6) | ((pix[6] & 0x01) << 5) | ((pix[7] & 0x01) << 4) | ((pix[4] & 0x02) << 2) | ((pix[5] & 0x02) << 1) | ((pix[6] & 0x02) << 0) | ((pix[7] & 0x02) >> 1);
			rom[lineaddr + 3] = ((pix[4] & 0x04) << 5) | ((pix[5] & 0x04) << 4) | ((pix[6] & 0x04) << 3) | ((pix[7] & 0x04) << 2) | ((pix[4] & 0x08) >> 0) | ((pix[5] & 0x08) >> 1) | ((pix[6] & 0x08) >> 2) | ((pix[7] & 0x08) >> 3);
		}
	}
}


uint8_t show_tile_data_019e[32 * 32] = {
0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0