...
# (Very) Rudimentary check for Evercade
## build environment...
if [[ "$CC" != "arm-linux-gnueabihf-gcc" || "$CXX" != "arm-linux-gnueabihf-g++" ]]
then
	echo "ERROR: Invalid build environment - please run"
	echo "'classic_set_env_evercade_<ggc_version>' before"
	echo "attempting to use this script, e.g.:"
	echo "> . classic_set_env_evercade_10"
	echo "> ./build_evercade.sh"
	exit 10
fi
...
# Perform build
OVERRIDE_CXX=$CXX OVERRIDE_CC=$CC PYTHON_EXECUTABLE=python \
	make -f Makefile.libretro CROSS_BUILD=1 LIBRETRO_OS="linux-arm" \
	TARGET=mame SUBTARGET=arcade \
	${FILTER_FILE:+ SOURCEFILTER="$FILTER_FILE"} \
	${DEBUG_OPT:+ "$DEBUG_OPT"} \
	${SYMBOLS_OPT:+ "$SYMBOLS_OPT"} \
	TARGETOS=linux PLATFORM=arm NOASM=1 OPENMP=0 LTO=1 \
	-j$NUM_THREADS \
